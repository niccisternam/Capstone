# Sistema Web para el Procesamiento Inteligente y Centralización de Documentos Tributarios

> Proyecto de Portafolio de Título (APT) – Duoc UC  


---

## 📌 Descripción del Proyecto
Plataforma web interna orientada a la centralización, digitalización y procesamiento automatizado de documentos tributarios de compra (facturas, boletas) y cotizaciones mediante el uso de Inteligencia Artificial multimodal. Incluye un módulo de validación asistida (*Human-in-the-loop*), panel de métricas de gasto en CLP y asistente conversacional.

---

## 👥 Equipo de Trabajo
* **Nicolás Cisterna Mena** – Jefe de Proyecto / Desarrollador Full-Stack 
* **Marcelo Vásquez** – Desarrollador Full-Stack  
* **Martín Godoy** – Desarrollador Full-Stack 
* **Fabián Saldaño** – Docente Patrocinador / Duoc UC

---

## 📂 Documentación – Entrega 1 (Fase 1)
# 📁 Evidencia Proyecto: Minuta de reuniones semanales
- Acta constitución proyecto
- Planilla de Requerimientos
- Carta Gantt/RoadMap 
- Documento ERS
- Documento Caso uso extendido
- Mockups  del sistema completo
- Presentación del Proyecto.pptx
# 📁 Evidencias Individuales: Contiene archivos solicitados separados por carpetas con nombre de los integrantes
# 📁 Evidencias Grupales: Contiene 1.4_APT122_FormativaFase1.docx / 1.5_GuiaEstudiante_Fase 1_Definicion Proyecto APT (Español)
---

## 🛠️ Stack Tecnológico Proyectado
* **Frontend:** React.js / Vue.js, Tailwind CSS
* **Backend:** Node.js / Python (FastAPI) – Arquitectura REST
* **Base de Datos:** Relacional (PostgreSQL / MySQL)
* **IA:** Modelos Multimodales pre-entrenados vía API (JSON estructurado)
